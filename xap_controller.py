...
from homeassistant.components.media_player import (
    ...
    SUPPORT_SELECT_SOURCE, MediaPlayerDevice, PLATFORM_SCHEMA, MEDIA_TYPE_MUSIC)

from homeassistant.const import (
    CONF_HOST, CONF_PORT, STATE_OFF, STATE_ON, STATE_UNKNOWN, STATE_IDLE, CONF_NAME)
...
SUPPORT_XAP_ZONE   = SUPPORT_VOLUME_MUTE | SUPPORT_VOLUME_SET | \
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend({
    ...
    vol.Required(CONF_ZONES): vol.Schema({cv.string: vol.All(cv.ensure_list,[int])}),
...
class XAPSource(MediaPlayerDevice):
    ...
    def set_volume_level(self, volume):
        ...
        volume = self._xapx00.setPropGain(self._input, volume, isAbsolute=1, group="I")
        self._volume = volume
    ...
    def turn_off(self):
        ...
        if self._xapx00.getMute(self._input) == 0:
            self._isMuted = self._xapx00.setMute(self._input, group="I", isMuted=1)
...
class XAPZone(MediaPlayerDevice):
    ...
    def __init__(self, hass, xapconn, sources, zone_name, outputs, unitCode=0):
        ...
        self.select_source(self._active_source)  #make sure sources synced across outputs
    ...
    def setDefaultLevel(self):
        ...
        for xOut in self._outputs:
            for source in self._sources:
                volume = self._xapx00.getMatrixLevel(self._sources[source],
                                                     self._outputs[xOut], self._defaultMatrixLevel)
    ...
    def get_source(self):
        ...
        for i in self._sources.keys():
            if i != SRC_OFF:
                z_state = self._xapx00.getMatrixRouting(self._sources[i], self._outputs[0])
    ...
    def _sync_volume_level(self):
        """set all level of all outputs in zone to the same level as the first one in zone"""
        if self._active_source != SRC_OFF:
            volume = self._xapx00.getPropGain(self._outputs[0], group="O", unitCode=self._unitCode)
            #volume = self._xapx00.getMatrixLevel(self._sources[self._active_source], self._outputs[0])
    ...
    def set_volume_level(self, volume):
        ...
        for output in self._outputs:
            ...
            volume = self._xapx00.setPropGain(output, volume, group="O", unitCode=self._unitCode)
            #self._xapx00.setMatrixLevel(self._sources[self._active_source], output, volume)
        self._volume = volume
    ...
    def mute_volume(self, mute=2):
        ...
        for output in self._outputs:
            muted = self._xapx00.setMute(output, group="O", isMuted=int(mute), unitCode=self._unitCode)
    ...
    def source_list(self):
        ...
        _LOGGER.debug("Source List, Active Source: {}".format(self._active_source))
